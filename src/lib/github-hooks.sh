...
function create_pull_request() {
    ...
    local JSON PR_NUMBER RESULT TRY_COUNT HAS_ERRORS

    set_github_origin_data

    # We would like to a 'github_query', but we need the whole JSON because,
    # OMG, for some reason, 'last_rest_status' does not appear reliable. We
    # get 200 responses. The body is still that of a failed call, though.
    JSON=`github_api POST /repos/$GITHUB_OWNER/$GITHUB_REPO/pulls '{"title": "Pull request for issue #'$ISSUE_NUMBER'","body": "Fixes #'$ISSUE_NUMBER'. Generated by conveyor-workflow.","head": "'$BRANCH_NAME'","base": "master"}'`
    RESULT=$?
    TRY_COUNT=1
	...
	if [ $TRY_COUNT -ge 5 ]; then
	    ...
	    HAS_ERRORS=`json_extract '["errors"]' "$JSON" | wc -l`
	    if [ $HAS_ERRORS -eq 0 ] && [ $RESULT -eq 0 ]; then
		PR_NUMBER=`json_extract '["number"]' "$JSON"`;
		if [[ "$PR_NUMBER" =~ ^[0-9]+$ ]]; then
		    # This is success; all else is just a different possible
		    # failure.
		    echo "$PR_NUMBER"
		    return 0
		fi
	...
	git push -q origin "$BRANCH_NAME":"$BRANCH_NAME"
	sleep 1 # Who knows if this useful.
	JSON=`github_api POST /repos/$GITHUB_OWNER/$GITHUB_REPO/pulls '{"title": "Pull request for issue #'$ISSUE_NUMBER'","body": "Fixes #'$ISSUE_NUMBER'. Generated by conveyor-workflow.","head": "'$BRANCH_NAME'","base": "master"}'`
	RESULT=$?
