...
function create_pull_request() {
    ...
    local PR_NUMBER RESULT TRY_COUNT

    set_github_origin_data

    PR_NUMBER=`github_query '["number"]' POST /repos/$GITHUB_OWNER/$GITHUB_REPO/pulls '{"title": "Pull request for issue #'$ISSUE_NUMBER'","body": "Fixes #'$ISSUE_NUMBER'. Generated by conveyor-workflow.","head": "'$BRANCH_NAME'","base": "master"}'`
    RESULT=$?
    TRY_COUNT=1
    while [ `last_rest_status` -eq 422 ] && [ $TRY_COUNT -lt 5 ]; do
