...
#* Feature: Checkout
#*
#* Scenario: Switching topics with local changes; spec. unstaged delete.
...
#*   And a file has been deleted from the working directory
...
rm README.repo
test_output 'git convey topics checkout existing-topic-b' '' "Current branch has unstaged changes; cowardly refusing to checkout topic 'existing-topic-b'." 1
CURRENT_BRANCH=`git rev-parse --abbrev-ref HEAD`
if [ x"$CURRENT_BRANCH" != x'topics-existing-topic-a' ]; then
