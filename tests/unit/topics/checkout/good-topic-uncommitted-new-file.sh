...
#*   And there are is a new, tracked, but uncommitted file in the working directory
...
touch foo
git add foo
test_output 'git convey topics checkout existing-topic-b' '' "Current branch has unncommitted changes; cowardly refusing to checkout topic 'existing-topic-b'." 1
CURRENT_BRANCH=`git rev-parse --abbrev-ref HEAD`
if [ x"$CURRENT_BRANCH" != x'topics-existing-topic-a' ]; then
